...
def objective_energy_harvesting(x_norm, args, debug=False):
    ...
    try:
        ...
        for annulus in rotor.annuli_lst:
            if hasattr(annulus, 'hist') and 'alpha' in annulus.hist:
                alphas.append(annulus.hist['alpha'][-1])
            elif hasattr(annulus, 'alpha'):
                alphas.append(annulus.alpha)
            
            if hasattr(annulus, 'a'):
                a_values.append(annulus.a)
            
            if hasattr(annulus, 'phi'):
                phi_values.append(annulus.phi)
        
        alphas = np.array(alphas) if alphas else np.array([])
        a_values = np.array(a_values) if a_values else np.array([])
...
def optimize_energy_harvesting():
    ...
    print(f"  Twist distribution: θ(r/R) = {at:.2f}·(r/R) + {bt-46:.2f}°")
    ...
    thrust, azimuthal, torque, final_power = rotor.calculate_integral(R=R, Uinf=Uinf, rho=rho)
    ...
    for annulus in rotor.annuli_lst:
        if hasattr(annulus, 'hist') and 'alpha' in annulus.hist:
            alphas.append(annulus.hist['alpha'][-1])
            radial_positions.append(annulus.r_R)
        elif hasattr(annulus, 'alpha'):
            alphas.append(annulus.alpha)
            radial_positions.append(annulus.r_R)
...
#Feasible Results:
#  Twist distribution: θ(r/R) = -54.66·(r/R) + 74.14°
#   Chord distribution: c(r/R)/R = 0.1755 + -0.0618·(r/R)