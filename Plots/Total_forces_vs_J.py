...
def forces_vs_J(save=False, exp_df=None, val_df=None):
    ...
    if exp_df is not None:
        plt.plot(exp_df['J'], exp_df['Total Thrust'], '-', label='T ', color='blue', linewidth=2)
        plt.plot(exp_df['J'], exp_df['Torque'], '--', label='Q', color='red', linewidth=2)
    
    # Plot validation data (dotted lines)
    if val_df is not None:
        plt.plot(val_df['J'], val_df['Thrust']*1000, ':', label='T (JavaProp)', color='blue', linewidth=2, marker='o', markersize=4)
        plt.plot(val_df['J'], val_df['Torque']*1000, ':', label='Q (JavaProp)', color='red', linewidth=2, marker='s', markersize=4)
    ...
    plt.tight_layout()
    if save:
        plt.savefig('Total forces vs J.png', dpi=300,)
    plt.show()
