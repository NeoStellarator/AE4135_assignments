...
def forces_vs_n(save=False, exp_df=None, val_df=None):
    """
    ...
    if exp_df is not None:
        plt.plot(exp_df['n_elem'], exp_df['Total Thrust'], '-', label='T ', color='blue', linewidth=2)
        plt.plot(exp_df['n_elem'], exp_df['Torque'], '--', label='Q', color='red', linewidth=2)
    
    
    
    plt.ylabel('Forces [N and N-m]')
    plt.xlabel('number of elements (n_elem)')
    # plt.title('Total Thrust and Torque vs Advance ratio')
    plt.legend(loc='upper right')
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    if save:
        plt.savefig('forces_span_dist.png', dpi=300,)
    plt.show()
